...
function setCalculation(){
	...
	if(startingMembership < 300){
		...
		var membershipRewards = parseInt(startingMembership) * 3;
		$('#membershipRewards').val(membershipRewards).digits();
		var bonus = startingMembership * 0.005;
...
function calcRewards(bonus, myDate, startingMembership, membershipRewards){
	...
	var rebuyPrev = 0;
	var decimalVal = 0.0;
	var dailyBonusRaw = startingMembership * 0.005;
	...
	if(rebuyPrev >= ribeye){
		membershipRewardAmount = membershipRewardAmount + (rebuyPrev * 3);	
	}
